...
<details>
<summary>Далее следуют инструкции, позволяющие обеспечить совместимость сборок и утилит BugBane.</summary>
...
**Внимание: в настоящий момент после сохранения дампов завершаются ВСЕ процессы afl-fuzz и tmux в пределах операционной системы.**<br>
...
Для Go используется единственная сборка (в папке gofuzz или gotest) и назначается на все выделенные ядра.<br>
...
Для каждого бага сохраняются такие сведения как заголовок issue/бага, место возникновения бага в исходном коде, команда запуска с конкретным семплом, вывод приложения (stdout+stderr), переменные окружения.<br>
